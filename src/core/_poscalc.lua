...
function __DanmakuArea:split(h1, h2, cloneArea)
...
function __DanmakuArea.getCollidingDuration(a1, a2, screenWidth)
...
function __BasePosCalculator:init(width, height)
...
function __BasePosCalculator:__recycleDanmakuAreas()
...
function __BasePosCalculator:_obtainDanmakuArea()
    return utils.popArrayElement(self.__mFreeDanmakuAreas) or __DanmakuArea:new()
end

function __BasePosCalculator:dispose()
    self:__recycleDanmakuAreas()
    utils.forEachArrayElement(self.__mFreeDanmakuAreas, utils.disposeSafely)
end


function __BasePosCalculator:__getCollisionScoreSum(iterTop, iterArea, newTop, area2)
    local scoreSum = 0
...
function __BasePosCalculator:__addDanmakuArea(iterArea, iterTop, area2, newTop)
...
function __BasePosCalculator:calculate(w, h, start, lifeTime)
    -- 区域位置全部用整数表示
    h = math.ceil(h)
...
function MovingPosCalculator:_doGetCollisionScore(area1, area2)
    return area1:getCollidingDuration(area2, self._mScreenWidth)
end


function MovingPosCalculator:_doInitDanmakuArea(w, h, start, lifeTime, outArea)
    local speed = 0
    if lifeTime == 0
    then
        -- 防止出现除零错误
...
function MovingPosCalculator:_doUpdateDanmakuArea(area1, area2)
...
function StaticPosCalculator:_doInitDanmakuArea(w, h, start, lifeTime, outArea)
...
function StaticPosCalculator:_doGetCollisionScore(area1, area2)
...
function StaticPosCalculator:_doUpdateDanmakuArea(area1, area2)
