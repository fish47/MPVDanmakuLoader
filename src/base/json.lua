...
{
    result                  = classlite.declareConstantField(nil),
    content                 = classlite.declareConstantField(nil),
    readIndex               = classlite.declareConstantField(0),
    stringBuf               = classlite.declareTableField(),
    keyStack                = classlite.declareTableField(),
    collectionStack         = classlite.declareTableField(),
    addItemFuncStack        = classlite.declareTableField(),
    parseItemListFuncStack  = classlite.declareTableField(),

    _reset = function(self, content)
        self.result = nil
        self.content = content
        self.readIndex = 0
        utils.clearTable(self.stringBuf)
        utils.clearTable(self.keyStack)
        utils.clearTable(self.collectionStack)
        utils.clearTable(self.addItemFuncStack)
        utils.clearTable(self.parseItemListFuncStack)
    end,
}
...
_onParseString = function(ctx)
    local buf = utils.clearTable(ctx.stringBuf)
    local content = ctx.content
...
_onParseConstant = function(ctx)
    ...
    for _, constName, val in utils.iteratePairsArray(_MAP_CONSTANT)
    do
